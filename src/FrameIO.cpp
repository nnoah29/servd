...
namespace
{
    ...
    {
        ClientFrame frame;
        std::span<std::byte> header_span{reinterpret_cast<std::byte*>(&frame.header), sizeof(FrameHeader)};
    ...
    {
        ClientFrame frame{};
        std::string header_line = co_await read_text_line(client_fd);
        ...
        std::string payload_line = co_await read_text_line(client_fd);
