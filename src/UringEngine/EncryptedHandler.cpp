...
namespace
{
        ...
        const std::vector<std::byte>& payload, Session& session)
    {
        ...
        SERVD_LOG(Logger::LogLevel::DEBUG, "[Encrypted] Cmd %u decrypted (%zu bytes)", inner_cmd, inner_payload.size());
        ...
        AesGcm& cipher, uint16_t inner_cmd, std::span<const std::byte> inner_payload, Session& session)
    {
        ...
        if (!ep) {
            SERVD_LOG(Logger::LogLevel::WARN, "[Encrypted] Unknown command: %u", inner_cmd);
            co_return;
        }
        ...
        if (!ok) {
            SERVD_LOG(Logger::LogLevel::WARN, "[Encrypted] Rejected cmd %u", inner_cmd); co_return;
        }
